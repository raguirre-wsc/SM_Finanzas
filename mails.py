# noinspection PyUnresolvedReferences
from openpyxl import Workbook

# noinspection PyUnresolvedReferences
import ctypes

# noinspection PyUnresolvedReferences
from tkinter import *

# noinspection PyUnresolvedReferences
from tkinter import ttk

# noinspection PyUnresolvedReferences
import openpyxl

# noinspection PyUnresolvedReferences
import arrow

# noinspection PyUnresolvedReferences
import numpy as np

# noinspection PyUnresolvedReferences
from auxiliar import auxiliar

# noinspection PyUnresolvedReferences
import pandas

# noinspection PyUnresolvedReferences
import xlrd

# noinspection PyUnresolvedReferences
import xlwings as xw

# noinspection PyUnresolvedReferences
import time

# noinspection PyUnresolvedReferences
from openpyxl import load_workbook

# noinspection PyUnresolvedReferences
import xlsxwriter

# noinspection PyUnresolvedReferences
import tkinter

# noinspection PyUnresolvedReferences
from tkinter import ttk

# noinspection PyUnresolvedReferences
from PIL import ImageTk, Image

# noinspection PyUnresolvedReferences
import win32com.client as win32

# noinspection PyUnresolvedReferences
import escritura

def mailTransfer():
    olApp=win32.Dispatch("Outlook.Application")
    olNS=olApp.GetNameSpace("MAPI")

    mailItem= olApp.CreateItem(0)

    mailItem.Subject="Transferencias urgentes "+arrow.now().format('DD')+"."+arrow.now().format('MM')+"."+arrow.now().format('YYYY')

    mailItem.BodyFormat=1

    mailItem.To= "JoseVicente.Siniga@swissmedical.com.ar;" \
             " matiasluis.zingarelli@swissmedical.com.ar;" \
             " CarlosAndres.Bello@swissmedical.com.ar;" \
             " sebastian.rodriguez@swissmedical.com.ar;" \
             " VanesaGiselle.Bon@swissmedical.com.ar;" \
             " Laila.Pelozo@swissmedical.com.ar;"\
             " Ayrton.deBrunoRe@swissmedical.com.ar;"\
             " Federico.Balestrelli@swissmedical.com.ar;"\
             " micaela.barberis@swissmedical.com.ar;" \
             " Sebastian.Moreira@swissmedical.com.ar;" \
             " marcosjavier.gonzalez@swissmedical.com.ar"

    escritura.TransAM()

    html=r"C:\Users\rodriaguirre\Desktop\libro.html"
    with open(html, "r") as file:
        data=file.read()

    mailItem.HTMLBody=data

    mailItem.Attachments.Add("C:/Users/rodriaguirre/OneDrive - Swiss Medical S.A/Documents/General/03. Posicion Financiera Diaria/02. Transferencias/" + arrow.now().format('YYYY') + "/" + arrow.now().format('MM') + ". " + auxiliar.nombreMes(str(arrow.now().format('MM'))) + "/Transferencias " + arrow.now().format('DD-MM-YYYY') + ".xlsm")

    mailItem.Display()


def mailOps():
    olApp = win32.Dispatch("Outlook.Application")
    olNS = olApp.GetNameSpace("MAPI")

    mailItem = olApp.CreateItem(0)

    mailItem.Subject = "Operaciones " + arrow.now().format('DD') + "." + arrow.now().format('MM') + "." + arrow.now().format('YYYY')

    mailItem.BodyFormat = 1

    mailItem.To = "JoseVicente.Siniga@swissmedical.com.ar;" \
                  " micaela.barberis@swissmedical.com.ar;" \
                  " Sebastian.Moreira@swissmedical.com.ar;" \
                  "daniel.bacchi@swissmedical.com.ar;" \
                  "Guillermo.Akris@swissmedical.com.ar;" \
                  "pablonicolas.fernandez@swissmedical.com.ar;" \
                  "SofiaMalena.Tortorella@swissmedical.com.ar;"\
                  "maximiliano.sosa@swissmedical.com.ar;" \
                  "Cecilia.Fiorentini@swissmedical.com.ar;"\
                  "ailin.dopazo@swissmedical.com.ar;" \
                  "augusto.ledesma@swissmedical.com.ar"

    mailItem.CC = "CarlosAndres.Bello@swissmedical.com.ar;" \
                  "Gustavo.Pizzoli@swissmedical.com.ar;" \
                  "enrique.pafundo@swissmedical.com.ar;" \
                  "marcosjavier.gonzalez@swissmedical.com.ar"

    mailItem.HTMLBody = """
    Estimados,<br>
    <br>
    A continuación detallo las operaciones del día de la fecha:<br>
    <br>
    <br>
    Gracias!
    <br>
    <br>
    <IMG SRC="data:image/png;base64,">
    """

    mailItem.Display()

def mailAcl():
    olApp = win32.Dispatch("Outlook.Application")
    olNS = olApp.GetNameSpace("MAPI")

    mailItem = olApp.CreateItem(0)

    mailItem.Subject = "Fondeo suscripciones " + arrow.now().format('DD') + "." + arrow.now().format('MM') + "." + arrow.now().format('YYYY')

    mailItem.BodyFormat = 1

    mailItem.To = "SofiaMalena.Tortorella@swissmedical.com.ar;"\
                  "maximiliano.sosa@swissmedical.com.ar;" \
                  "Cecilia.Fiorentini@swissmedical.com.ar;"\

    mailItem.HTMLBody = """
    Aclaraciones:<br>
    <br>
    <br>
    Saludos.
    <br>
    <br>
    <IMG SRC="data:image/png;base64,">
    """

    mailItem.Display()

def mailContrapartes():
    olApp = win32.Dispatch("Outlook.Application")

    dest={
        "ADCAP": "FZiade@ad-cap.com.ar; mtokatlian@ad-cap.com.ar; mesalocal@ad-cap.com.ar",
        "ICBC": "nicolas.stescovich@icbc.com.ar; gustavo.borchardt@icbc.com.ar",
        "MARIVA": "Sergio.Tula@marivafondos.com.ar; ComercialMAM@marivafondos.com.ar; wolfensong@mariva.com.ar; Agustin.Tapia@marivafondos.com.ar",
        "BACS": "FBRUNDI@BACS.COM.AR; solicitudes@torontotrust.com.ar",
        "PATAGONIA": "pesquivel@bancopatagonia.com.ar; MGeragalet@bancopatagonia.com.ar; lali@bancopatagonia.com.ar; lgaluppo@bancopatagonia.com.ar; Fondos@bancopatagonia.com.ar",
        "CONSULTATIO": "mdeluca@consultatioinvestments.com",
        "SUPERVIELLE": "Diego.Sanges@supervielle.com.ar>; FCI@supervielle.com.ar; luisalberto.gomez@supervielle.com.ar",
        "AR PARTNERS": "andrea.visintin@arpartners.com.ar; mariano.parras@arpartners.com.ar; emiliano.pellegrino@arpartners.com.ar",
        "GALICIA": "guido.m.low@bancogalicia.com.ar; back.financiero@bancogalicia.com.ar",
        "MEGAINVER": "gustavo.chicharro@megainver.com; comercial@megainver.com; operaciones@megaqm.com.ar",
        "SCHRODERS": "Pablo.Wilenski@schroders.com; Paula.Bennati@Schroders.com; Mariano.Fiorito@schroders.com; pedro.cavallo@schroders.com; Operaciones.BA@Schroders.com; Natacha.Canay@schroders.com",
        "SANTANDER": "fondos@santanderrio.com.ar; INSTITUCIONAL_SALES@santander.com.ar; dgaido@santanderrio.com.ar; aburtea@santander.com.ar; gohammoe@santanderrio.com.ar",
        "SBS": "fbv@gruposbs.com;SCisa@gruposbs.com;gd@gruposbs.com;cc@gruposbs.com;jru@gruposbs.com",
        "GALILEO": "vpadin@galileoargentina.com.ar; pgiustinian@galileoargentina.com.ar; operaciones@galileoargentina.com.ar; lmartinez@galileoargentina.com.ar; mivancich@galileoargentina.com.ar; nserin@galileoargentina.com.ar; mrigal@galileoargentina.com.ar",
        "BNP PARIBAS":"ayelen.moreno@consultant.bnpparibas.com; horacio.bonafine@ar.bnpparibas.com; pablo.diazmathe@ar.bnpparibas.com",
        "STONE X": "giselda.ortega@intlfcstone.com; fabian.jungman@intlfcstone.com; Milagros.Satorre@StoneX.com",
    }


    dir_op=r"//xfs/rulero/Sector/SMG_Administracion_y_Finanzas/Inversiones/Estructura_Vieja/Gerencia de Inversiones/Front Office New/Liquidez/Operaciones/Monitor Operaciones 221206.xlsm"
    pd=pandas.read_excel(dir_op, sheet_name="Nueva hoja soporte")

    cont_base=list(dest.keys())
    cont_dia=list(pd['CONTRAPARTE'].unique())

    cont_intersect=list(set(cont_base) & set(cont_dia))


    for i in cont_intersect:
        sub_pd = pd.loc[pd["CONTRAPARTE"] == i]

        mailItem = olApp.CreateItem(0)

        mailItem.CC = "Adrian.Alfonso@swissmedical.com.ar; Guillermo.Akris@swissmedical.com.ar; ailin.dopazo@swissmedical.com.ar; daniel.bacchi@swissmedical.com.ar; enrique.pafundo@swissmedical.com.ar;SofiaMalena.Tortorella@swissmedical.com.ar; PabloNicolas.Fernandez@swissmedical.com.ar; aldana.perez@swissmedical.com.ar; augusto.ledesma@swissmedical.com.ar"

        mailItem.Subject = "SMG - " + i + " - Operaciones del día " + arrow.now().format('DD') + "." + arrow.now().format('MM') + "." + arrow.now().format('YYYY')

        mailItem.BodyFormat = 1

        mailItem.To = dest[i]

        escritura.mailsContra()

        html = "C:/Users/rodriaguirre/Desktop/Mails/"+i+".html"
        with open(html, "r") as file:
            data = file.read()

        mailItem.HTMLBody = data

        mailItem.Display()







